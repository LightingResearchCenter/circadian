function varargout = setLogInfo(log_infoPath,varargin)
%SETLOGINFO Edits a Daysimeter log_info.txt file
...
%   EXAMPLE:
%       parameters = struct;
%       parameters.status = io.daysimeterStatus.Start;
%       parameters.startDateTime = now;
%       parameters.loggingInterval = 30;
%
%       io.setLogInfo('log_info.txt',parameters)
...
if numel(statusObj) == 1
    TF = isa(statusObj,'io.daysimeterStatus');
else
    TF = false;
end
